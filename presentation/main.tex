...
\begin{frame}{Risk-Averse DQN: Shen (2014) Update Rule}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item $u$ is \emph{concave}: large \alert{negative} $\delta$
              amplified $\to$ agent strongly avoids bad outcomes
        \item Value function turns pessimistic about the tail
      \end{itemize}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \smallskip
      \footnotesize
      For $\lambda<1$: concavity of $u(x)=\operatorname{sgn}(x)|x|^\lambda$ amplifies negative TD-errors and compresses positive ones $\Rightarrow$ asymmetric penalty on downside
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{block}{Kaggle Competition (2021)}
        \small
        \texttt{kaggle.com/c/}\\
        \texttt{optiver-realized-volatility-prediction}\\[6pt]
        Goal: predict short-term realized volatility\\
        from LOB snapshots.\\[4pt]
        We \emph{repurpose} the LOB data as a\\
...
\begin{frame}{Summary \& Future Work}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      CVaR is \textbf{evaluation metric}, not objective.
    \end{column}
    \begin{column}{0.46\textwidth}
      \textbf{Future work:}
      \begin{enumerate}
...
\begin{frame}[plain]
  \centering
  \vspace{1.5cm}
  {\Huge\color{myblue}\textbf{Thank you!}}\\[14pt]
  {\large Questions?}\\[24pt]
  \footnotesize
  Code: \texttt{real\_data/optiver\_agent\_v3.py}\\[4pt]
  Reproducible on NVIDIA RTX 3080 in $\approx$30 min.
\end{frame}
