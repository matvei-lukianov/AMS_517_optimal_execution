...
\begin{frame}{MDP Formulation}
  \begin{itemize}
    \item \textbf{State:} $S_t = (q_t/Q,\; \tilde{s}_t,\; t/T,\; \text{imbalance}_t,\; \text{depth}_t)$ — 5 continuous features
    \item \textbf{Action:} $a_t \in \{0\%,\,10\%,\ldots,100\%\}$ of remaining inventory $q_t$
    \item \textbf{Inventory Dynamics:} $q_{t+1} = q_t - a_t$
    \item \textbf{Reward} (execution quality in bps):
    $$r_t = \frac{\bar{p}_t^{\text{exec}} - p_t^{\text{mid}}}{p_0^{\text{mid}}} \times 10^4$$
    \item \textbf{Terminal penalty:} unexecuted shares dumped at 1.5\% discount
    \item \textbf{Market impact:} LOB sweep (temporary) + Almgren-Chriss (permanent):
    $$\Delta p_t^{\text{perm}} = \gamma\, a_t, \quad \gamma = 2\times 10^{-5}$$
  \end{itemize}
\end{frame}
...
\begin{frame}{Out-of-Sample Results — 112 Stocks, 42,890 Episodes}
  \footnotesize
  \textbf{Cost} = shortfall vs perfect execution at initial mid-price, in basis points:
  $\displaystyle\text{cost} = \frac{Q\cdot p_0^{\text{mid}} - \text{cash received}}{Q\cdot p_0^{\text{mid}}}\times 10^4$
  \quad Higher = worse.
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{RA vs RN — opposite to Shen}
        $$\Delta\text{CVaR}_{95} = 127.7 - 56.8 = \mathbf{+70.9}\text{ bps}$$
        Shen (2014): RA \emph{reduces} CVaR by $10$--$15$ bps vs RN.\\
        We find RA \alert{worse} — TD-error proxy insufficient\\
        on real LOB data $\Rightarrow$ motivates distributional RL.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
            ...
            \item[\checkmark] Shen (2014) replicated with GPU DQN on 112 stocks
             \item[\checkmark] RA vs RN CVaR: $\mathbf{+70.9}$~bps (RA worse — proxy fails)
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{alertblock}{Main takeaway}
        TD-error utility \emph{fails} to reduce CVaR on real LOB data.
        Both DQN agents beat VWAP on mean cost (\textbf{81\%} of episodes).
        Direct tail optimisation (QR-DQN) is the necessary next step.
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
